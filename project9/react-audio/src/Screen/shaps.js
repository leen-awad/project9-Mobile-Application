...
import soundShaps from "../../DB/shapSound";
...
const Alphabet = ({ navigation }) => {
  ...
  const [arrRequire, SetReq] = useState(soundShaps[0].letterSound);
  const [selectedId, setSelectedId] = useState(null);
  const [coins, setCoins] = useState(coins);
  const [Diamond, setDiamond] = useState(soundShaps[0].totalDiamond);
  const [Levels, setLevels] = useState(soundShaps[0].levels);
  ...
  const Item = ({ item }) => (
    <TouchableOpacity
      onPress={() => {
        playSound(item, item.soundShaps), increaseCoins(item);
      }}
    >
  ...
  useEffect(() => {
    saveData(coins);
    let count = soundShaps[0].letterSound.filter((letter) => letter.buy).length;
