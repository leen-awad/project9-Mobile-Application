...
const Alphabet = ({ navigation }) => {
  ...
  const Diamond_KEY = "@save_diamond";
  const [selectLetter, setSelectLetter] = useState("");
  const { width, height } = Dimensions.get("window");
  const [arrRequire, SetReq] = useState(soundLetter[0].letterSound);
  const [selectedId, setSelectedId] = useState(null);
  const [coins, setCoins] = useState(coins);
  const [Diamond, setDiamond] = useState(Diamond);
  const [Levels, setLevels] = useState(soundLetter[0].levels);
  ...
  const Item = ({ item }) => (
    <TouchableOpacity
      onPress={() => {
        playSound(item, item.soundLetter), increaseCoins(item);
      }}
    >
  ...
  useEffect(() => {
    saveData(coins, Diamond);
    // saveData(Diamond);
    let count = soundLetter[0].letterSound.filter((letter) => letter.buy)
      .length;
  ...
  const cheakData = async () => {
    try {
      let coins = await AsyncStorage.getItem(STORAGE_KEY);
      let daimond = await AsyncStorage.getItem(Diamond_KEY);

      if (coins == null || daimond == null) {
  ...
  const saveData = async () => {
    try {
      ...
      await AsyncStorage.setItem(Diamond_KEY, Diamond.toString());
      // console.log(
  ...
  const readData = async () => {
    try {
      ...
      const userDaimond = await AsyncStorage.getItem("@save_diamond");

      console.log("Read", userCoins);
      if (userCoins !== null || userDaimond !== null) {
        setCoins(parseInt(userCoins));
        setDiamond(parseInt(userDaimond));
      }
    } catch (e) {
      console.log(e);
    }
